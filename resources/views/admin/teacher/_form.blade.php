...
if(isset($Teacher)&&$Teacher->gender==$item->id):
...
@else
<form action="{{action('App\Http\Controllers\Admin\TeacherController@update', $id)}}" method="POST" enctype="multipart/form-data">
...
<div class="col-xs-12 col-md-4">
                <div class="form-group">
                    <label for="user-name" class="form-control-label">{{ __('Teacher Name') }}</label>
                    <div class="@if($errors->has('name'))border border-danger rounded-3 @endif">
                        <input class="form-control" value="@if(isset($Teacher)){{$Teacher->name}}@else{{old('name')}}@endif" type="text" placeholder="Teacher Name" id="user-name" name="name">
                            @if($errors->has('name'))
...
<div class="col-xs-12 col-md-4">
                <div class="form-group">
                    <label for="user-name" class="form-control-label">{{ __(' Teacher No.') }}</label>
                    <div class="@if($errors->has('admission_no'))border border-danger rounded-3 @endif">
                        <input class="form-control" value="@if(isset($Teacher)){{$Teacher->admission_no}}@else{{old('admission_no')}}@endif" type="text" placeholder=" Teacher No." id="user-name" name="admission_no">
    ...
    <div class="col-xs-12 col-md-4">
                <div class="form-group">
                    ...
                    <div class="@if($errors->has('dob'))border border-danger rounded-3 @endif">
                        <input class="form-control" value="@if(isset($Teacher)){{$Teacher->dob}}@else{{old('dob')}}@endif" type="text" placeholder="Date of Birth" id="input-from_date" name="dob">
                            @if($errors->has('dob'))
...
<div class="col-xs-12 col-md-4">
                <div class="form-group">
                    ...
                    <div class="@if($errors->has('phone'))border border-danger rounded-3 @endif">
                        <input class="form-control" value="@if(isset($Teacher)){{$Teacher->phone}}@else{{old('phone')}}@endif" type="text" placeholder="Phone Number" id="user-name" name="phone">
                            @if($errors->has('phone'))
    ...
    <div class="col-xs-12 col-md-4">
                <div class="form-group">
                    ...
                    <div class="@if($errors->has('email'))border border-danger rounded-3 @endif">
                        <input class="form-control" value="@if(isset($Teacher)){{$Teacher->email}}@else{{old('email')}}@endif" type="text" placeholder="Email Address" id="user-name" name="email">
                            @if($errors->has('email'))
...
<input class="form-control" type="file" name="logo">
          ...
          <?php if(strlen($Teacher->file) >2): ?>
          
          <img src="{{ asset("uploads/".$Teacher->file) }}" width="100%" alt="logo" >
          <?php endif; ?>
