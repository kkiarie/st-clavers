...
Amount Payable {{number_format($Amount,2)}}
          ...
          <a href="{{ URL::to("/fee-structure/create/$FeeStructure->id") }}" >
            <button class="btn btn-success mt-4"><i class="fas fa-plus"></i> New fee Item </button>
          </a>
 @if($Amount>0)
          <a href="{{ URL::to("/fee-structure-complete/$FeeStructure->id") }}" >
            <button onclick="return confirm('Are you sure ?')" class="btn btn-success mt-4"><i class="fas fa-plus"></i> Complete </button>
          </a>
          @endif
...
<form action="{{action('App\Http\Controllers\Admin\FeeStructureController@destroy', $item->id)}}" method="POST" role="form">
            {{ csrf_field() }}
