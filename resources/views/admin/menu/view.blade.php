                    ...
                    <div class="d-flex flex-row justify-content-between">
                        <div>
                            <h5 class="mb-0">{{$Menu->title}}</h5>
                                                   ...
                                                   <a href="{{ URL::to("/menu-setup/$Menu->id/edit") }}" >
            <button class="btn btn-primary mt-4"><i class="far fa-edit"></i> Edit {{$Menu->title}}</button>
          </a>

                    <?php if($Menu->level==0):?>
          <a href="{{ URL::to("/menu-setup/create/1/$Menu->id") }}" >
            <button class="btn btn-success mt-4"><i class="fas fa-plus"></i> New Menu Item </button>
          </a>
          <?php else: ?>
            <a href="{{ URL::to("/menu-setup/create/2/$Menu->id") }}" >
            <button class="btn btn-success mt-4"><i class="fas fa-plus"></i> New Menu Item </button>
          </a>
          <?php endif;?>

                    <form action="{{action('App\Http\Controllers\Admin\MenuController@destroy', $Menu->id)}}" method="POST" role="form">
            {{ csrf_field() }}
        <input name="_method" type="hidden" value="DELETE">
        <button class="btn btn-danger mt-2" onclick=" return confirm('Are you sure ?')"><i class="far fa-trash-alt"></i> Delete {{$Menu->title}}</button>
        </form>  
                        </div>
